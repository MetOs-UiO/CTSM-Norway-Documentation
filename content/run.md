...
First, you need to change the global input data directory variable inside the script (standard global CTSM dataset) as below:
    ...
    dir_inputdata='/cluster/shared/noresm/inputdata/'  (somelines after region.create-tag())
...
and add this file location in the python script `subset_data.py`. Make sure you have created a conda environment for python including xarray, netcdf4 installed or loaded on FRAM or SAGA.
```
